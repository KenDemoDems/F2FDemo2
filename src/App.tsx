  ...
  const fetchInventory = async () => {
    ...
    try {
      ...
      if (result.error) {
        ...
        fetchIngredientImages(result.inventory.map(item => item.name));
      }
    } catch (err: any) {
      ...
      try {
        ...
        if (data.photos && data.photos.length > 0) {
          setIngredientImages(prev => ({
            ...prev,
            [name]: data.photos[0].src.medium, // Use medium size, adjust if needed
          }));
        } else {
          console.warn(`No food-related image found for ${name}, using default`);
          setIngredientImages(prev => ({
            ...prev,
            [name]: '🥕', // Fallback to emoji if no image
          }));
        }
      } catch (error) {
        ...
        setIngredientImages(prev => ({
          ...prev,
          [name]: '🥕', // Fallback to emoji on error
        }));
      }
    }
  };
  ...
  const handleAddToBin = async (itemId: string) => {
    ...
    try {
      const newWasteItem: WasteItem = {
        ...
        image: categoryIcons[itemToMove.category || 'general']?.props.children || '🥕',
                          ...
                          <div className="flex items-center space-x-3 sm:space-x-4 pr-10 sm:pr-14 mb-4">
                            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white/90 rounded-full flex items-center justify-center shadow-lg flex-shrink-0 overflow-hidden">
                              {ingredientImages[item.name] ? (
                                ...
                                <span className="text-lg">🥕</span> // Fallback while image loads
                              )}
                            </div>
                            <div className="min-w-0 flex-1">
                              <h3 className="font-bold text-white text-sm sm:text-lg truncate">{item.name}</h3>
                              <p className="text-white/90 text-xs sm:text-sm font-medium">
                                {item.daysLeft} Days left • {item.quantity} {item.unit}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-3">
                            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-white/90 rounded-full flex items-center justify-center shadow-sm flex-shrink-0 overflow-hidden">
                              {ingredientImages[item.name] ? (
                                ...
                                <span className="text-base sm:text-lg">{item.image}</span> // Fallback to original image
                              )}
                            </div>
                      ...
                      <div>
                        ...
                        <div className="text-center">
                          ...
                          <div className="flex items-center justify-center space-x-4 text-sm text-gray-500 mb-4">
                            ...
                            <Badge className="bg-emerald-100 text-emerald-700">{selectedLeftoverRecipe.difficulty}</Badge>
                          </div>
                        ...
                        <div>
                          ...
                          <div className="space-y-3 text-sm text-gray-700">
                            <div className="flex items-start space-x-2">
                              <div className="w-5 h-5 bg-emerald-100 text-emerald-700 rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0 mt-0.5">1</div>
                              <span>Check your leftover ingredients for quality and freshness</span>
                            </div>
                            <div className="flex items-start space-x-2">
                              <div className="w-5 h-5 bg-emerald-100 text-emerald-700 rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0 mt-0.5">2</div>
                              <span>Prepare ingredients by washing, chopping, and seasoning as needed</span>
                            </div>
                            <div className="flex items-start space-x-2">
                              <div className="w-5 h-5 bg-emerald-100 text-emerald-700 rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0 mt-0.5">3</div>
